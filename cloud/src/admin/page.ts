...
(function () {
  ...
  function renderOps(o) {
    $('view-ops').innerHTML =
      ...
      '<div class="panel"><h2>Grenzwerte</h2><p class="sub">Tagesbudget, Rate-Limits, Modelle und die Schwellenwerte der Warnungen sind Umgebungsvariablen, kein gespeicherter Zustand — sie werden in <code>wrangler.jsonc</code> geändert und ausgerollt. So bleibt der veröffentlichte Quelltext frei von den Zahlen, gegen die jemand austarieren würde.</p><div class="kv" style="margin-top:10px"><dt>Tagesbudget</dt><dd>' + fmtUsd(o.budget.limitUsd) + '</dd><dt>Heute verbraucht</dt><dd>' + fmtUsd4(o.budget.spentUsd) + '</dd><dt>Zustand</dt><dd>' + (o.budget.killed ? 'gestoppt' : 'läuft') + '</dd></div></div>';
